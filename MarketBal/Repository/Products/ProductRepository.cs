...
namespace MarketBal.Repository.Products
{
            ...
            var whereClauses = new List<string>
    {
        ...
        "p.OrganizationId = @OrganizationId"
            ...
            if (!string.IsNullOrWhiteSpace(searchValue))
            {
                ...
                OR bm.ModelName LIKE @Search
                OR b.BrandName LIKE @Search
                OR sc.SubCategoryName LIKE @Search
            )");
            ...
            // COUNT QUERY (always WITHOUT paging)
            string countQuery = $@"
        SELECT COUNT(*) 
        FROM INV.Products p
        JOIN INV.BrandModels bm ON p.BrandModelId = bm.BrandModelId
        JOIN INV.Brands b ON bm.BrandId = b.BrandId
    ...
    ";
       ...
       LEFT JOIN INV.BrandModels bm ON p.BrandModelId = bm.BrandModelId
        LEFT JOIN INV.Brands b ON bm.BrandId = b.BrandId
        JOIN INV.SubCategory sc ON p.SubCategoryId = sc.SubCategoryId
    ...
    ";
        ...
        public async Task<Guid> AddProduct(ProductVM product, int? productType = 1)
        {
            try
            {
...
Left JOIN Inv.Colors c ON pv.ColorId = c.ColorId
...
Left JOIN INV.Brands b ON p.BrandId = b.BrandId
LEFT JOIN INV.BranchStock bs ON pv.VariantId = bs.ProductVariantId
